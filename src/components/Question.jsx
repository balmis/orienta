...
export default function Question({
  ...
  return (
    <div className="flex flex-col px-4 md:px-10 pt-6 md:pt-10 pb-8 gap-5 md:gap-7 min-h-full">
      {/* Bloque / categoría */}
      <div>
        <span
      ...
      {pregunta.nota && (
        <div className="flex gap-2 bg-blue-50 border border-blue-100 rounded-xl p-3">
          <span className="text-base md:text-lg shrink-0">ℹ️</span>
          <p className="text-xs md:text-sm text-blue-700 leading-relaxed">{pregunta.nota}</p>
        </div>
      )}
      ...
      <div className="flex gap-3 mt-auto pt-2">
        {puedeRetroceder && (
          <button
            onClick={onBack}
            className="flex-none px-5 md:px-7 py-3 md:py-4 rounded-2xl border-2 border-gray-200 text-gray-600 font-semibold text-sm md:text-base transition-all active:scale-[0.97] hover:border-gray-300"
          >
            ← Atrás
          </button>
        )}
        <button
          ...
          className={[
            'flex-1 py-3 md:py-4 rounded-2xl font-bold text-sm md:text-base transition-all active:scale-[0.97]',
            puedeAvanzar
